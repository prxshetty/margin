...
Providers, styles, and ComfyUI workflows are configured in **Settings → Images**:
...
- **Styles** — Style suffixes appended to your prompt. Shipped styles (`Cinematic`, `Illustration`) can be edited, hidden (Restore brings them back), or reset; add your own below the list. `None` is never deletable and means no suffix.
